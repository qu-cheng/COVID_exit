...
par.sample.all <- read.csv("Data/par_best_worst.csv")  # read in parameter values for best- and worst-case sceanrio by locations
...
for(rep.i in 1:2)
{
  ...
  result.current<- do.call(rbind, results) %>%
    data.frame() %>%
    mutate(repi = rep.i, Scenario = "baseline")
  
  result.all <- rbind(result.all, result.current)
  
  save(result.all, file = 'Results/China_best_worst_Sens.rda')
  ...
  results <- mclapply(1:(nc), function(i){
    ...
    bed.remain <- copy(bed.remain.original)
    Agents.current <- copy(Agents)
    model_run_VW_Delay(Agents.current, bed.remain,
                       R0 = 5,
                       rho = 1,
                       vac.ve = "between",
                       import.rate = 70/57752557*Total.popsize,
  ...
  result.current<- do.call(rbind, results) %>%
    data.frame() %>%
    mutate(repi = rep.i, Scenario = "R05")
  ...
  result.current<- do.call(rbind, results) %>%
    data.frame() %>%
    mutate(repi = rep.i, Scenario = "R010")
  ...
  results <- mclapply(1:(nc), function(i){
    ...
    model_run_VW_Delay(Agents.current, bed.remain,
                       ...
                       surv.rate.M = 0.75,
                       home.mild.reduced = 0.8,
                       child.sus = c(0.52, 0.72),
                       home.nocare.h = 9)
  }, mc.cores = nc)
  
  result.current<- do.call(rbind, results) %>%
    data.frame() %>%
    mutate(repi = rep.i, Scenario = "ChildSus")
  ...
  result.current<- do.call(rbind, results) %>%
    data.frame() %>%
    mutate(repi = rep.i, Scenario = "AsympInfectivity")
  ...
  result.current<- do.call(rbind, results) %>%
    data.frame() %>%
    mutate(repi = rep.i, Scenario = "SelfIsolation")
  ...
  result.current<- do.call(rbind, results) %>%
    data.frame() %>%
    mutate(repi = rep.i, Scenario = "VacOptimistic")
  ...
  result.current<- do.call(rbind, results) %>%
    data.frame() %>%
    mutate(repi = rep.i, Scenario = "VacPessimistic")
  ...
  results <- mclapply(1:(nc), function(i){
    ...
    model_run_VW_Delay(Agents.current, bed.remain,
                       ...
                       vac.ve = "between",
                       import.rate = 70/57752557*Total.popsize,
                       surv.rate.M = 0.5625,   # 0.75 coverage
                       home.mild.reduced = 0.8,
                       child.sus = c(1, 1),
                       home.nocare.h = 9)
  }, mc.cores = nc)
  ...
  result.current<- do.call(rbind, results) %>%
    data.frame() %>%
    mutate(repi = rep.i, Scenario = "mild0.5625")
  ...
  result.current<- do.call(rbind, results) %>%
    data.frame() %>%
    mutate(repi = rep.i, Scenario = "home.h5")
  ...
  result.current<- do.call(rbind, results) %>%
    data.frame() %>%
    mutate(repi = rep.i, Scenario = "home.h2")
