...
par.sample.all <- read.csv("Data/par_best_worst_change.csv")  # read in parameter values for best- and worst-case sceanrio by locations
...
result.all <- NULL
for(rep.i in 1:nrow(par.sample))
{
  ...
  results <- mclapply(1:(nc), function(i){
    ...
    model_run(Agents.current, bed.remain,
                       R0 = 7,
                       rho = 1,
                       vac.ve = "optimistic",    # change to "between" for the baseline scenario
  ...
  save(result.all, file = 'China_best_worst_change_optimistic.rda')
}
